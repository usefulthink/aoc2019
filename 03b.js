...
const [cableA, cableB] = require('fs')
  .readFileSync('03-in.txt', 'utf-8')
  .split('\n');
  ...
  for (let i = 2; i < ls1.length; i += 2) {
    ...
    for (let j = 2; j < ls2.length; j += 2) {
      ...
      if (intersection) {
        ...
        const length =
          lengthA +
          distance(a[0], a[1], x, y) +
          lengthB +
          distance(b[0], b[1], x, y);
...
assert.equal(getClosestDistance('R8,U5,L5,D3', 'U7,R6,D4,L4'), 30);
assert.equal(
  getClosestDistance(
